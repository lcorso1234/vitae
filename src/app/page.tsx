...
function VitaeBuilderContent() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('vitae-data-v5');
    ...
    if (company || role || problem || solution) {
      setData(prev => {
        ...
        localStorage.setItem('vitae-data-v5', JSON.stringify(newData));
        return newData;
      });
    } else if (saved) {
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              onClick={async () => {
                try {
                  const localData = localStorage.getItem('vitae-data-v5') || localStorage.getItem('vitae-data-v4') || localStorage.getItem('vitae-data');
